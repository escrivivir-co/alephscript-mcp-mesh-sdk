...
export abstract class BaseMCPServer {
        ...
        this.app.get("/resources/:resourceId(*)", async (req, res) => {
            try {
                ...
                if (resourceId.startsWith("stategraphs/")) {
                    ...
                    l.v(
                        `${this.config.name}: Detected stategraphs request`,
                        { resourceId }
                    );
                }

                // Use MCP server's native resource handling
                const resourceResult = await this.handleResourceRequest(
                    effectiveResourceId
                );
            ...
            if (resourceHandlers) {
                ...
                for (const [handlerKey, handler] of resourceHandlers) {
                    ...
                    if (
                        ...
                        (resourceId.includes("stategraphs/") &&
                            handlerKey.includes("stategraph")) ||
                        (resourceId.includes("x-plus-1-game") &&
                            handlerKey.includes("x-plus-1-game"))
                    ) {
                        l.v(
                ...
                l.v(
                    `${this.config.name}: No pattern match found, trying URI matching`
                );
                ...
                for (const [handlerKey, handler] of resourceHandlers) {
                    try {
                        ...
                        l.v(
                            `${this.config.name}: Checking URI: ${handlerUri} against ${resourceId}`
                        );
                        if (
                            handlerUri &&
                            (handlerUri.includes(resourceId) ||
                                resourceId.includes(
                                    handlerUri.split("://")[1] || handlerUri
                                ))
                        ) {
                            l.v(
                                `${this.config.name}: Found URI match: ${resourceId} -> ${handlerUri}`
                            );
                            return await handler.callback(request.params);
                        }
                    } catch (error) {
                        ...
                        l.v(
                            `${this.config.name}: Handler ${handlerKey} failed, continuing...`
                        );
                    }
                }
            }
    ...
    async initialize(): Promise<void> {
        l.v(`${this.config.name}: Initializing MCP server`);
    ...
    async start(): Promise<void> {
        ...
        l.i("Basic MCP Server instance created, starting... with bot support");
        ...
        this.app.get("/mcp", async (req, res) => {
            l.v(`${this.config.name}: Received GET MCP request`);
    ...
    async shutdown(): Promise<void> {
        l.v(`${this.config.name}: Shutting down MCP server`);
