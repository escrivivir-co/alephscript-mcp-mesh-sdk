...
import { z } from "zod";
import { DEFAULT_STATE_MACHINE_MCP_SERVER_CONFIG, MCP_ORFEO_STATE_BOT } from "@configs/DEFAULT_STATE_MACHINE_MCP_SERVER_CONFIG";


import { l } from "./Logger";
import { AppConfig, DEFAULT_APP_CONFIG } from "@configs/app.config";
...
export class MCPStateMachineServer extends BaseMCPServer {
    ...
    constructor(public appConfig: AppConfig = DEFAULT_APP_CONFIG) {
        ...
        l.i("MCPStateMachineServer Server instance created, starting... with bot support. 1");
        ...
        l.i("MCPStateMachineServer Server instance created, starting... with bot support. 2");
        ...
        l.i("MCPStateMachineServer Server instance created, starting... with bot support. 3");

        // Initialize OrfeoBot
        l.i("MCPStateMachineServer Server instance created, initOrfeoBot.");
        this.initOrfeoBot();
    }
    ...
    private initOrfeoBot(): void {
       /* try {
            ...
            l.i("MCPStateMachineServer Server instance created, initOrfeoBot. 1");
            ...
            this.orfeoBot.initTriggersDefinition.push(() => {
                ...
                l.i("MCPStateMachineServer OrfeoBot. ASYNC TRIGGER");
                ...
                this.orfeoBot.io.onAny((eventName: string, ...args: any[]) => {
                    l.i(`Event received: ${eventName}`, args);
                });
                
                // You can also use specific wildcard patterns if needed
                this.orfeoBot.io.on("*", (event: any, data: any) => {
                    l.i(`Wildcard event: ${event}`, data);
                });
                
                
                l.i("OrfeoBot initialized and connected to AlephScript server", {
                    botName: this.name,
                    room: ROOM_NAME
                });
            });

            l.i("OrfeoBot client created successfully");
        } catch (error) {
            l.e("Failed to initialize OrfeoBot", { error });
        }*/
    }
    ...
    protected setupServerSpecifics(): void {
        ...
        if (this.orfeoBot) {
            l.i("Starting OrfeoBot connection...");
     ...
     */
    public updateConversation(message: {
        id: string;
        sender: string;
        message: string;
    }): void {
    ...
    public updateAvailableAgents(agents: string[]): void {
        ...
        this.gameState.currentPhase =
            agents.length > 0 ? "selecting_agent" : "idle";
    ...
    private setupTools(): void {
        ...
        this.server.tool(
            ...
            async ({ reason, metadata }) => {
                ...
                l.v(
                    `X advanced: ${previousX} → ${this.state.x}`,
                    { reason, metadata }
                );
                            ...
                            text: JSON.stringify(
                                {
                                    ...
                                    resetCount: this.state.resetCount,
                                    sessionDuration:
                                        Date.now() - this.state.sessionStart,
                                    totalAdvancements:
                                        this.state.advancementHistory.length,
                                    currentStreak:
        ...
        this.server.tool(
            ...
            async ({ userInput, context }) => {
                ...
                if (
                    ["yes", "y", "true", "reset", "consumed"].some((word) =>
                        input.includes(word)
                    )
                ) {
                    decision = "reset";
                    ...
                    ["no", "n", "false", "advance", "continue"].some((word) =>
                        input.includes(word)
                    )
                ) {
                    decision = "advance";
                    confidence = 0.9;
                    reasoning =
                        "User indicated no consumption, triggering advancement";
                } else {
                    decision = "clarify";
                    confidence = 0.1;
        ...
        this.server.tool(
            ...
            async ({ graphId, userId }) => {
                ...
                l.v("State load requested", { graphId, userId });
                ...
                context: z
                    .object({
                        simulateTyping: z
                            .boolean()
                            .optional()
                            .describe("Simulate typing delay"),
                        delayMs: z
                            .number()
                            .optional()
        ...
        this.server.tool(
            ...
            "Select a specific agent from available postulations",
            {
                agentId: z
                    .string()
                    .describe("ID or name of the agent to select"),
                reason: z
                    .string()
                    .optional()
                    .describe("Reason for selecting this agent"),
            },
        ...
        this.server.tool(
            ...
            "Answer yes/no to the critical question from JusticeBot",
            {
                answer: z
                    .enum(["yes", "no"])
                    .describe("Answer to the critical question"),
                reasoning: z
                    .string()
                    .optional()
                    .describe("Reasoning behind the answer"),
            },
        ...
        this.server.tool(
            ...
            {},
            async () => {
                l.v("Conversation data requested", {
                    threadLength: this.gameState.conversationThread.length,
                });
                            ...
                            text: JSON.stringify(
                                {
                                    success: true,
                                    conversation:
                                        this.gameState.conversationThread,
                                    messageCount:
                                        this.gameState.conversationThread
                                            .length,
                                    timestamp: Date.now(),
        ...
        this.server.tool(
            ...
            {},
            async () => {
                l.v("Available postulations requested", {
                    agentCount: this.gameState.availableAgents.length,
                });
                            ...
                            text: JSON.stringify(
                                {
                                    success: true,
                                    availableAgents:
                                        this.gameState.availableAgents,
        ...
        this.server.tool(
            ...
            "Toggle between manual and automatic simulator mode",
            {
                mode: z
                    .enum(["on", "off", "toggle"])
                    .optional()
                    .describe("Set mode explicitly or toggle"),
            },
            async ({ mode }) => {
                const command: RemoteCommand = {
                ...
                if (mode === "on") {
                    ...
                    this.gameState.simulatorMode =
                        !this.gameState.simulatorMode;
                }

                this.gameState.lastAction = `toggle_simulator: ${
                    this.gameState.simulatorMode ? "on" : "off"
                }`;

                l.v("Simulator mode toggled", {
        ...
        this.server.tool(
            ...
            "Update the shared game state from the UI",
            {
                ...
                messageCount: z
                    .number()
                    .optional()
                    .describe("Current message count"),
                availableAgents: z
                    .array(z.string())
                    .optional()
        ...
        this.server.tool(
            ...
            "Add a message to the conversation thread",
            {
                sender: z
                    .string()
                    .describe("Message sender (user, agent name, etc.)"),
                message: z.string().describe("Message content"),
                messageId: z
                    .string()
                    .optional()
                    .describe("Optional message ID"),
            },
            async ({ sender, message, messageId }) => {
                ...
                if (this.gameState.conversationThread.length > 50) {
                    this.gameState.conversationThread =
                        this.gameState.conversationThread.slice(-50);
                }
                ...
                l.v("Conversation message added", {
                    sender,
                    messageLength: message.length,
                    threadSize: this.gameState.conversationThread.length,
                });
                            ...
                            text: JSON.stringify(
                                {
                                    ...
                                    internalState: {
                                        x: this.state.x,
                                        lastAdvancement:
                                            this.state.lastAdvancement,
                                        resetCount: this.state.resetCount,
                                        sessionStart: this.state.sessionStart,
                                        advancementHistory:
                                            this.state.advancementHistory,
                                    },
        ...
        this.server.tool(
            ...
            async () => {
                ...
                l.v("Console output placeholder accessed");
                            ...
                            text: JSON.stringify(
                                {
                                    success: true,
                                    message:
                                        "Console reading not yet connected to UI instance",
                                    placeholder: true,
                                    implementationNeeded:
                                        "Connect to ConsoleGamificationUI.getCurrentOutput()",
                                    timestamp: Date.now(),
                                },
                                null,
                                2
        ...
        this.server.tool(
            ...
            {},
            async () => {
                l.v("Current prompt placeholder accessed");
                            ...
                            text: JSON.stringify(
                                {
                                    success: true,
                                    message:
                                        "Prompt reading not yet connected to UI instance",
                                    placeholder: true,
                                    implementationNeeded:
                                        "Connect to ConsoleGamificationUI.getCurrentPrompt()",
                                    timestamp: Date.now(),
                                },
                                null,
                                2
        ...
        this.server.tool(
            ...
            {},
            async () => {
                l.v("UI status placeholder accessed");
                            ...
                            text: JSON.stringify(
                                {
                                    success: true,
                                    message:
                                        "UI status reading not yet connected to UI instance",
                                    placeholder: true,
                                    implementationNeeded:
                                        "Connect to ConsoleGamificationUI.getUIStatus()",
                                    timestamp: Date.now(),
                                },
                                null,
                                2
        ...
        this.server.tool(
            ...
            {},
            async () => {
                l.v("Interaction state placeholder accessed");
                            ...
                            text: JSON.stringify(
                                {
                                    success: true,
                                    message:
                                        "Interaction state reading not yet connected to UI instance",
                                    placeholder: true,
                                    implementationNeeded:
                                        "Connect to ConsoleGamificationUI.getInteractionState()",
                                    currentPhase: this.gameState.currentPhase,
                                    availableAgents:
                                        this.gameState.availableAgents,
                                    timestamp: Date.now(),
...
`;
        ...
        // StateGraph resource
        l.v("Registering StateGraph resource", {
            id: "stategraphs/x-plus-1-game",
        });
        this.server.resource(
                    ...
                    contents: [
                        {
                            ...
                            text: JSON.stringify(
                                {
                                    queueSize: this.commandQueue.length,
                                    pendingCommands: this.commandQueue.map(
                                        (cmd) => ({
                                            id: cmd.id,
                                            type: cmd.type,
                                    ...
                                    isProcessing:
                                        this.gameState.isWaitingForRemote,
                                    timestamp: Date.now(),
                                },
                                null,
                                2
        ...
        l.i("X+1 resources setup completed");
    }
                    ...
                    messages: [
                        {
                            ...
                            content: {
                                type: "text",
                                text:
                                    `⚖️ **JusticeBot - The Moment of Truth**\n\n` +
                                    `Current X: ${x}\n` +
                                    `Reset count: ${this.state.resetCount}\n\n` +
                                    `**Did you consume today, do I reset?**\n\n` +
                                    `Answer honestly:\n` +
        ...
        this.server.prompt(
            ...
            async () => {
                const sessionDuration = Math.floor(
                    (Date.now() - this.state.sessionStart) / 60000
                );
                const totalActions = this.state.advancementHistory.length;
                const advancements = this.state.advancementHistory.filter(
                    (h) => h.to > h.from
                ).length;
        ...
        this.server.prompt(
            ...
            "Guide for controlling the X+1 game remotely via MCP",
            {
                context: z
                    .string()
                    .optional()
                    .describe("Current context or situation"),
                phase: z.string().optional().describe("Current game phase"),
            },
            async ({ context, phase }) => {
                return {
                    messages: [
                        {
                            ...
                            content: {
                                type: "text",
                                text:
                                    `🎮 **Remote Control Guide for X+1 Game**\n\n` +
                                    `**Available Commands:**\n` +
        ...
        this.server.prompt(
            ...
            "Help with agent selection and critical decisions",
            {
                availableAgents: z
                    .string()
                    .optional()
                    .describe("Available agent names (comma-separated)"),
                    ...
                    messages: [
                        {
                            ...
                            content: {
                                ...
                                text:
                                    ...
                                    `• Recent Context: ${
                                        conversationContext || "No context"
                                    }\n\n` +
                                    `**Agent Personalities:**\n` +
        ...
        this.server.prompt(
            ...
            "Analyze current conversation state and suggest next actions",
            {
                conversationSummary: z
                    .string()
                    .optional()
                    .describe("Summary of conversation messages"),
                messageCount: z
                    .string()
                    .optional()
                    .describe("Current message count"),
                maxMessages: z
                    .string()
                    .optional()
                    .describe("Maximum messages allowed"),
            },
                    ...
                    messages: [
                        {
                            ...
                            content: {
                                ...
                                text:
                                    ...
                                    `${
                                        remaining > 5
                                            ? "• Continue conversation, plenty of messages left"
                                            : remaining > 2
                                            ? "• Consider moving toward decision phase"
                                            : "• Conversation nearing end, prepare for critical question"
                                    }\n` +
                                    `${
                                        count === 0
                                            ? "• Start with DionisioBot or ApoloBot for opening"
                                            : ""
                                    }\n` +
                                    `${
                                        remaining <= 1
                                            ? "• JusticeBot should ask critical question next"
                                            : ""
                                    }\n\n` +
