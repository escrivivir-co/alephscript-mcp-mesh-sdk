 ...
 */
export type UserSimulatorPersonality =
    | "cautious"
    | "balanced"
    | "risk_taker"
    | "passive";
 ...
 */
export class XPlus1ControlPlugin
    extends BaseDevOpsPlugin
    implements IDevOpsPlugin
{
            ...
            resources: [
                {
                    ...
                    description:
                        "Current status and configuration of UserSimulator",
                ...
                },
                {
                    ...
                    description:
                        "Analysis of current game state and recommended actions",
                    mimeType: "application/json",
                },
                {
                    ...
                    description:
                        "History of UserSimulator actions and decisions",
                    mimeType: "application/json",
                },
            ],
            prompts: [
                {
                    ...
                    description:
                        "Guide for controlling UserSimulator via MCP tools",
                    parameters: {
    ...
    async initialize(context: PluginContext): Promise<void> {
        ...
        if (!this.mcpAdapter) {
            this.log(
                "warn",
                "No MCP adapter available - plugin functionality will be limited"
            );
        }
    ...
    async setupMCPHandlers(context: PluginContext): Promise<void> {
        ...
        server.tool(
            ...
            "Change UserSimulator personality and behavior",
            {
                personality: z
                    ...
                    .describe("UserSimulator personality"),
                reason: z
                    .string()
                    .optional()
                    .describe("Reason for personality change"),
            },
            async ({
                personality,
                reason,
            }: {
                personality: UserSimulatorPersonality;
                reason?: string;
            }) => {
                this.updateActivity();
                ...
                try {
                    ...
                    this.log(
                        "info",
                        `UserSimulator personality changed to ${personality}`,
                        { reason }
                    );
                    ...
                    this.log("error", "Failed to set user personality", {
                        error,
                        personality,
                    });
                    throw error;
        ...
        server.tool(
            ...
            "Simulate user consumption decision intelligently",
            {
                forceDecision: z
                    .enum(["yes", "no", "clarify", "auto"])
                    .optional()
                    .describe("Force specific decision or use auto"),
                context: z
                    .object({
                        currentX: z.number().optional(),
                        messageCount: z.number().optional(),
                    ...
                    .optional()
                    .describe("Game context for decision"),
            },
            async ({
                forceDecision,
                context: gameContext,
                ...
                try {
                                ...
                                text: JSON.stringify(
                                    {
                                        success: true,
                                        message:
                                            "UserSimulator activated for automatic decision",
                                        simulatorMode: "on",
                                        context: gameContext,
                                        result,
                                    },
                    ...
                    this.log("error", "Failed to simulate user decision", {
                        error,
                    });
                    throw error;
                }
        ...
        server.tool(
            ...
            "Select specific agent for next message",
            {
                ...
                reasoning: z
                    .string()
                    .optional()
                    .describe("Reasoning for selection"),
            },
            async ({
                agentId,
                reasoning,
                ...
                try {
                    ...
                    const result = await mcpAdapter.executeTool(
                        ...
                        "select_agent",
                        {
                            ...
                            reason:
                                reasoning ||
                                `DevOps controlled selection via XPlus1 Control Plugin`,
                        }
                    );

                    this.log(
                        "info",
                        `Agent ${agentId} selected via DevOps control`,
                        { reasoning }
                    );
                    ...
                    this.log("error", "Failed to select agent", {
                        error,
                        agentId,
                    });
                    throw error;
        ...
        server.tool(
            ...
            "Enable/disable automatic UserSimulator mode",
            {
                mode: z
                    .enum(["on", "off", "toggle"])
                    .describe("Simulator mode control"),
                duration: z
                    .number()
                    .optional()
                    .describe("Duration in milliseconds for temporary mode"),
            },
            async ({
                mode,
                duration,
                ...
                try {
                    ...
                    if (duration && mode === "on") {
                        setTimeout(async () => {
                            try {
                                ...
                                this.log(
                                    "info",
                                    `Simulator mode automatically disabled after ${duration}ms`
                                );
                            } catch (error) {
                                this.log(
                                    "error",
                                    "Failed to disable simulator after timeout",
                                    { error }
                                );
                            }
                        }, duration);
                                ...
                                text: JSON.stringify(
                                    {
                                        ...
                                        simulatorMode: mode,
                                        duration,
                                        message: `Simulator mode ${mode}${
                                            duration ? ` for ${duration}ms` : ""
                                        }`,
                                        result,
                                    },
                                    null,
                    ...
                    this.log("error", "Failed to control simulator mode", {
                        error,
                    });
                    throw error;
                }
        ...
        server.tool(
            ...
            "Get current UserSimulator status and statistics",
            {
                includeHistory: z
                    .boolean()
                    .optional()
                    .describe("Include activity history"),
            },
            async ({ includeHistory }: { includeHistory?: boolean }) => {
                ...
                try {
                    ...
                    this.log("error", "Failed to get simulator status", {
                        error,
                    });
                    throw error;
                }
        ...
        server.tool(
            ...
            async ({
                ...
                try {
                    ...
                    const [xStatus, gameState, conversation, prompt] =
                        await Promise.all([
                    ...
                    this.log("error", "Failed to analyze game context", {
                        error,
                    });
                    throw error;
                }
        ...
        server.resource(
            ...
            async () => {
                try {
                    ...
                    this.log(
                        "error",
                        "Failed to get simulator status resource",
                        { error }
                    );
                    throw error;
                }
        ...
        server.prompt(
            ...
            "UserSimulator control guide with current context",
            {
                currentContext: z
                    .string()
                    .optional()
                    .describe("Current game context as JSON string"),
            },
            async (variables: { currentContext?: string }) => {
                let contextObj;
                try {
                    contextObj = variables?.currentContext
                        ? JSON.parse(variables.currentContext)
                        : undefined;
                } catch {
...
*Plugin: ${this.config.name} v${this.config.version}*`;
    ...
    private detectGamePhase(prompt: any): string {
        ...
        if (promptText.includes("available for automated"))
    ...
    private calculateUrgency(
        ...
        try {
            ...
            const maxMessages =
                this.parseResponse(gameState)?.maxMessages || 10;
    ...
    private recommendNextAction(context: any): string {
        ...
        switch (phase) {
            case "decision":
                return "simulate_user_decision";
            case "agent_selection":
                return "simulate_agent_selection";
            case "simulator_ready":
                return "control_simulator_mode";
            default:
                return "analyze_game_context";
        }
    }
