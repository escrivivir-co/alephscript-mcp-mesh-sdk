...
import { PluginConfig } from "@/managers";
import { IDevOpsPlugin, PluginContext, PluginResult } from "./IDevOpsPlugin";
...
export class DevOpsPluginManager {
    ...
    async registerPlugin(
        ...
        if (this.registry.has(config.id)) {
            l.w(`Plugin ${config.id} already registered, skipping`);
            return;
        }
        ...
        l.i(`Plugin ${config.name} (${config.id}) registered`);
    ...
    async initializePlugin(pluginId: string): Promise<void> {
        ...
        if (entry.active) {
            l.w(`Plugin ${pluginId} already initialized`);
            return;
        }
        ...
        try {
            ...
            l.e(`Failed to initialize plugin ${pluginId}`, {
                error,
            });
            throw error;
        }
    ...
    async unloadPlugin(pluginId: string): Promise<void> {
        const entry = this.registry.get(pluginId);
        if (!entry) {
            l.w(`Plugin ${pluginId} not found for unloading`);
            return;
        }
        ...
        try {
            ...
            l.i(`Plugin ${pluginId} unloaded successfully`);
        } catch (error) {
            l.e(`Error unloading plugin ${pluginId}`, { error });
            throw error;
        }
    }
    ...
    async executePluginCommand(
        ...
        try {
            ...
            l.e(`Plugin ${pluginId} command ${command} failed`, {
                error,
            });
            return {
                success: false,
