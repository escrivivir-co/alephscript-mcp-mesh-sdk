 ...
 * 
 * l.v(`MCPClientDriver: Tool ${toolName} executed successfully`, { serverId, executionTime: mcpResponse.executionTime });
      return mcpResponse.content;
    } catch (error) {
      l.e(`MCPClientDriver: Tool execution failed:`, { serverId, toolName, error });compatibility with Runtime
 */
...
import { MCPEventType, MCPToolResponse } from "./MCPTypes";
...
export class MCPClientDriver extends EventEmitter implements IMCPDriver {
    ...
    async executeTool(
        ...
        try {
            ...
            l.e(`MCPClientDriver: Tool execution failed:`, {
                serverId,
                toolName,
                error,
            });
    ...
    async getResource(
        ...
        try {
            ...
            l.e(`MCPClientDriver: Resource retrieval failed:`, {
                serverId,
                resourceId,
    ...
    async close(): Promise<void> {
        ...
        l.i("MCPClientDriver: All connections closed");
    }
    ...
    async saveState(serverId: string, state: any): Promise<void> {
        try {
            ...
            l.e("MCPClientDriver: Error saving state:", { error });
            throw error;
        }
    }
    ...
    private emitMCPEvent(event: MCPEvent): void {
        ...
        // Log event
        l.v(`MCP Event: ${event.type}:${event.action}`, {
        ...
        const interval = setInterval(async () => {
            try {
                ...
                if (!wasHealthy) {
                    this.healthStatus.set(serverId, true);
                    this.emitMCPEvent({
                        type: MCPEventType.HEALTH_CHECK,
                        action: "connected",
                        serverId,
