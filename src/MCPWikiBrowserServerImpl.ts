...
import { DEFAULT_WIKI_MCP_SERVER_CONFIG, MCP_EURIDICE_WIKI_BOT } from "./configs/DEFAULT_WIKI_MCP_SERVER_CONFIG";
import { l } from "./Logger";
import { AppConfig, DEFAULT_APP_CONFIG } from "./configs/app.config";
import { AlephScriptClient } from "./libs/alephscript-client";
import { getHash, IUserDetails } from "./MCPStateMachineServerImpl";
...
export class MCPWikiBrowserServer extends BaseMCPServer {
    ...
    private readonly WIKIPEDIA_API_BASE =
    ...
    constructor(public appConfig: AppConfig = DEFAULT_APP_CONFIG) {
        ...
        console.log(
            `   • Max Age: ${this.cache.maxAge / (1000 * 60 * 60)} hours`
        );
    ...
    private async initializeCache(): Promise<void> {
        ...
        try {
            console.log(
                `📁 WikiMCP: Creating cache directory at: ${this.cache.directory}`
            );
            ...
            l.i(
                `WikiMCP: Cache directory initialized at ${this.cache.directory}`
            );
        } catch (error) {
            console.error(
                `❌ WikiMCP: Failed to initialize cache directory at: ${this.cache.directory}`
            );
            console.error(`   • Error: ${error}`);
            l.e("WikiMCP: Failed to initialize cache directory", {
                error,
            });
    ...
    private async getCachedData(cacheKey: string): Promise<any | null> {
        ...
        try {
            ...
            l.i(`WikiMCP: Cache hit for key ${cacheKey}`);
            return entry.data;
        } catch (error) {
    ...
    private async setCachedData(
        ...
        try {
            ...
            l.i(`WikiMCP: Data cached with key ${cacheKey}`);

            // Check cache size and cleanup if needed
            await this.cleanupCache();
        } catch (error) {
            l.e(`WikiMCP: Failed to cache data for key ${cacheKey}`, {
                error,
            });
        }
    }
        ...
        try {
            ...
            if (totalSize > maxSizeBytes) {
                ...
                for (const { file, path: filePath, stats } of fileStats) {
                    ...
                    l.i(`WikiMCP: Removed old cache file ${file}`);
                }
            }
        } catch (error) {
            l.e("WikiMCP: Failed to cleanup cache", { error });
        }
    }
    ...
    private async clearCache(): Promise<void> {
        ...
        try {
            ...
            l.i(`WikiMCP: Cleared ${cacheFiles.length} cache files`);
        } catch (error) {
            l.e("WikiMCP: Failed to clear cache", { error });
            throw error;
        }
    }
    ...
    private async cachedRequest(url: string, config: any = {}): Promise<any> {
        ...
        try {
            l.i(`WikiMCP: Cache miss, fetching from ${url}`);
            ...
            const response = await axios.get(url, {
                ...
                headers: {
                    ...config.headers,
                    "User-Agent":
                        "WikiMCPBrowser/1.0 (https://github.com/escrivivir-co/state-machine-mcp-driver)",
                },
            });
            ...
            l.i(`WikiMCP: Response cached for future use`);
    ...
    private setupTools(): void {
        ...
        this.server.tool(
            ...
            "Load a Wikipedia article by title with full content and metadata",
            {
                title: z
                    .string()
                    .describe("Article title to load from Wikipedia"),
                includeImages: z
                    .boolean()
                    .optional()
                    .describe("Include thumbnail images in response"),
                language: z
                    .string()
                    .optional()
            ...
            async ({ title, includeImages = false, language = "en" }) => {
                try {
                    l.i(
                        `WikiMCP: Loading article "${title}" from ${language}.wikipedia.org`
                    );
                                    ...
                                    text: JSON.stringify(
                                        {
                                            ...
                                            message:
                                                "This is a disambiguation page. Please be more specific with the article title.",
                                            suggestions:
                                                page.extract
                                                    .match(/\[\[([^\]]+)\]\]/g)
                                                    ?.slice(0, 5) || [],
                    ...
                    try {
                        const contentResponse = await this.cachedRequest(
                            `https://${language}.wikipedia.org/api/rest_v1/page/html/${encodeURIComponent(
                                title
                            )}`
                        );
                        ...
                        l.w(
                            `WikiMCP: Could not fetch full content for ${title}, using summary`
                        );
                    ...
                    l.i(
                        `WikiMCP: Successfully loaded article "${title}" (${fullContent.length} chars)`
                    );
                                ...
                                text: JSON.stringify(
                                    {
                                        ...
                                        article: {
                                            ...
                                            wordCount:
                                                fullContent.split(" ").length,
                                            readingTimeMinutes: Math.ceil(
                                                fullContent.split(" ").length /
                                                    200
                                            ),
                                            images: includeImages
                                                ? [article.thumbnail].filter(
                                                      Boolean
                                                  )
                                                : undefined,
                                        },
                    ...
                    l.e(`WikiMCP: Failed to load article "${title}"`, {
                        error,
                    });
                                ...
                                text: JSON.stringify(
                                    {
                                        ...
                                        title: title,
                                        suggestion:
                                            "Try checking the spelling or use the search_wikipedia tool to find similar articles",
                                        errorDetails:
        ...
        this.server.tool(
            ...
            "Search Wikipedia for articles matching a query",
            {
                query: z
                    .string()
                    .describe("Search query for Wikipedia articles"),
                limit: z
                    .number()
                    .optional()
                    .describe(
            ...
            async ({ query, limit = 10, language = "en" }) => {
                try {
                    l.i(
                        `WikiMCP: Searching Wikipedia for "${query}" (limit: ${limit})`
                    );
                    ...
                    const totalHits =
                        searchResponse.data.query?.searchinfo?.totalhits || 0;

                    // Update session
                    this.session.searchHistory.push(query);

                    l.i(
                        `WikiMCP: Found ${results.length} results for "${query}" (${totalHits} total)`
                    );
                    ...
                    l.e(`WikiMCP: Search failed for "${query}"`, {
                        error,
                    });
        ...
        this.server.tool(
            ...
            "Get a random Wikipedia article for discovery",
            {
                ...
                namespace: z
                    .number()
                    .optional()
                    .describe("Wikipedia namespace (0 = articles, default: 0)"),
            },
            async ({ language = "en", namespace = 0 }) => {
                try {
                    l.i(
                        `WikiMCP: Getting random article from ${language}.wikipedia.org`
                    );
                    ...
                    l.e(`WikiMCP: Failed to get random article`, {
                        error,
                    });
        ...
        this.server.tool(
            ...
            "Get categories for a Wikipedia article to understand its themes",
            {
                title: z
                    .string()
                    .describe("Article title to get categories for"),
                language: z
                    .string()
                    .optional()
                    .describe("Wikipedia language code (default: en)"),
            ...
            async ({ title, language = "en" }) => {
                try {
                    l.i(`WikiMCP: Getting categories for "${title}"`);
                    ...
                    l.e(
                        `WikiMCP: Failed to get categories for "${title}"`,
                        { error }
                    );
        ...
        this.server.tool(
            ...
            "Clear the Wikipedia content cache to free up space",
            {
                confirm: z
                    .boolean()
                    .optional()
                    .describe("Confirm cache deletion (default: false)"),
            },
            async ({ confirm = false }) => {
                try {
                    if (!confirm) {
                                    ...
                                    text: JSON.stringify(
                                        {
                                            success: false,
                                            message:
                                                "Cache clear cancelled. Set confirm=true to proceed.",
                                            currentStats:
                                                await this.getCacheStatistics(),
                                        },
                                        null,
                                        2
                                    ),
                    ...
                    l.i("WikiMCP: Cache cleared manually");
                                ...
                                text: JSON.stringify(
                                    {
                                        ...
                                        cacheStats: stats,
                                        recommendations:
                                            this.getCacheRecommendations(stats),
                                    },
                                    null,
                                    2
                                ),
                            ...
                            text: JSON.stringify(
                                {
                                    ...
                                    articlesVisited:
                                        this.session.articlesVisited,
                                    articlesCount:
                                        this.session.articlesVisited.length,
                                    currentArticle:
                                        this.session.currentArticle?.title ||
                                        null,
                                    searchHistory: this.session.searchHistory,
                                    totalReadingTime:
                                        this.session.totalReadingTime,
                                    averageArticleTime:
                                        this.session.articlesVisited.length > 0
                                            ? this.session.totalReadingTime /
                                              this.session.articlesVisited
                                                  .length
                                            : 0,
                                    doomScrollingRisk:
                                        this.calculateDoomScrollingRisk(),
        ...
        this.server.prompt(
            ...
            "Dionisio's cosmic journey prompt using real Wikipedia content",
            {
                currentArticle: z
                    .string()
                    .optional()
                    .describe("Current article title"),
                articlesVisited: z
                    .string()
                    .optional()
                    .describe("Number of articles visited"),
            },
            async ({ currentArticle, articlesVisited }) => {
                ...
                const count =
                    articlesVisited ||
                    this.session.articlesVisited.length.toString();
                    ...
                    messages: [
                        {
                            ...
                            content: {
                                type: "text",
                                text:
                                    `🌌 **Dionisio's Cosmic Journey Through Wikipedia**\n\n` +
        ...
        this.server.prompt(
            ...
            async ({ currentArticle, sessionDuration }) => {
                ...
                const duration =
                    sessionDuration ||
                    Math.floor(
                        (Date.now() - this.session.startTime) / 60000
                    ).toString();
                    ...
                    messages: [
                        {
                            ...
                            content: {
                                type: "text",
                                text:
                                    `☀️ **Apolo's Beacon of Wisdom Through Wikipedia**\n\n` +
    ...
    private calculateDoomScrollingRisk(): "low" | "medium" | "high" {
        ...
        const articlesPerMinute =
            this.session.articlesVisited.length / Math.max(sessionDuration, 1);

        if (articlesPerMinute > 3 || this.session.articlesVisited.length > 20) {
            return "high";
        } else if (
            articlesPerMinute > 1.5 ||
            this.session.articlesVisited.length > 10
        ) {
            return "medium";
        } else {
            return "low";
    ...
    private async getCacheStatistics(): Promise<any> {
        ...
        try {
            ...
            const sortedByTime = fileStats.sort(
            ...
            return {
                ...
                newestEntry:
                    sortedByTime[sortedByTime.length - 1]?.mtime.toISOString(),
                utilizationPercent: Math.round(
                    (totalSize / (this.cache.maxSize * 1024 * 1024)) * 100
                ),
            };
        } catch (error) {
            return {
    ...
    private getCacheRecommendations(stats: any): string[] {
        ...
        if (stats.utilizationPercent > 90) {
            recommendations.push(
                "Cache is nearly full. Consider clearing old entries or increasing max size."
            );
        } else if (stats.utilizationPercent > 70) {
            recommendations.push(
                "Cache utilization is high. Monitor for performance."
            );
        }

        if (stats.totalFiles === 0) {
            recommendations.push(
                "Cache is empty. Start browsing Wikipedia to populate it."
            );
        } else if (stats.totalFiles < 10) {
            recommendations.push(
                "Cache has few entries. More browsing will improve performance."
            );
        }
