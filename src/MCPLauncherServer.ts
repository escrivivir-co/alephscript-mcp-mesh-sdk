...
import { DEFAULT_STATE_MACHINE_MCP_SERVER_CONFIG } from "@/configs/DEFAULT_STATE_MACHINE_MCP_SERVER_CONFIG";
...
export class MCPLauncherServer extends BaseMCPServer {
        ...
        this.server.tool(
            ...
            async ({ healthCheck = true }) => {
                try {
                    ...
                    l.v(`MCP Launcher: Launched all servers`, {
                        results,
                    });
        ...
        this.server.tool(
            ...
            async ({ serverId }) => {
                try {
                    ...
                    l.e(`MCP Launcher: Health check failed`, {
                        serverId,
                        error,
                    });
    ...
    private async saveVSCodeMCPConfig(
        ...
        try {
            ...
            l.e(`MCP Launcher: Failed to save VS Code config`, {
                error,
                outputPath,
            });
            return false;
    ...
    private async launchServer(
        ...
        l.v(`MCP Launcher: Starting ${config.name}`, {
        ...
        serverProcess.on('spawn', () => {
            l.i(`MCP Launcher: Process ${config.id} spawned successfully ${tsxCmd} ${args.join(' ')} Pid: ${serverProcess.pid}`);
        ...
        serverProcess.on('error', (error: any) => {
            l.i(`MCP Launcher: Process ${config.id} spawn error`, {
                error: error.message,
                stack: error.stack,
        ...
        serverProcess.on('exit', (code: any, signal: any) => {
            l.v(`MCP Launcher: Process ${config.id} exited`, {
                pid: serverProcess.pid,
                code,
                signal,
        ...
        serverProcess.on('close', (code: any, signal: any) => {
            l.i(`MCP Launcher: Process ${config.id} closed`, {
                pid: serverProcess.pid,
                code,
                signal
            });
        ...
        serverProcess.on('disconnect', () => {
            l.v(`MCP Launcher: Process ${config.id} disconnected`, {
                pid: serverProcess.pid
            });
        });
        ...
        if (serverProcess.stdout) {
            serverProcess.stdout.on('data', (data: any) => {
                ...
                if (output) {
                    l.v(`MCP Launcher: ${config.id} stdout`, {
                        pid: serverProcess.pid,
                        data: output,
            ...
            serverProcess.stdout.on('error', (error: any) => {
                l.e(`MCP Launcher: ${config.id} stdout error`, {
                    pid: serverProcess.pid,
                    error: error.message
                });
            });
            ...
            serverProcess.stdout.on('end', () => {
                l.v(`MCP Launcher: ${config.id} stdout ended`, {
                    pid: serverProcess.pid
                });
            });
        }
        ...
        if (serverProcess.stderr) {
            serverProcess.stderr.on('data', (data: any) => {
                const output = data.toString().trim();
                if (output) {
                    l.v(`MCP Launcher: ${config.id} stderr`, {
                        pid: serverProcess.pid,
                        data: output,
                        length: output.length,
                        isError: true
            ...
            serverProcess.stderr.on('error', (error: any) => {
                l.e(`MCP Launcher: ${config.id} stderr error`, {
                    pid: serverProcess.pid,
                    error: error.message
                });
            });
            ...
            serverProcess.stderr.on('end', () => {
                l.v(`MCP Launcher: ${config.id} stderr ended`, {
                    pid: serverProcess.pid
                });
            });
        }
        ...
        if (serverProcess.stdin) {
            serverProcess.stdin.on('error', (error: any) => {
                l.e(`MCP Launcher: ${config.id} stdin error`, {
                    pid: serverProcess.pid,
                    error: error.message
                });
            });
            ...
            serverProcess.stdin.on('close', () => {
                l.v(`MCP Launcher: ${config.id} stdin closed`, {
                    pid: serverProcess.pid
                });
            });
        }

        l.v(`MCP Launcher: ${config.id} process setup complete`, {
            pid: serverProcess.pid,
    ...
    private setupProcessHandlers(
        ...
        process.on("error", (error) => {
            l.e(`MCP Launcher: Process ${config.id} error`, {
                error,
            });
    ...
    private async autoRestartServer(
        ...
        l.v(`MCP Launcher: Auto-restarting ${config.id}`, {
    ...
    async shutdown(): Promise<void> {
        l.i("MCP Launcher: Shutting down all managed servers");
        ...
        for (const [serverId, process] of this.processes) {
            try {
                ...
                l.v(`MCP Launcher: Error stopping ${serverId}`, {
                    error,
                });
            }
        }
